...
def run_tests():
    ...
    if str(elephant) == "Animal(name=Dumbo, species=Elephant, age=10, height=3.06, width=4.08, preferred_habitat=Jungle)":
        score += 1
    else:
        print(f'Test 6 Failed: {elephant}')
        print(str(elephant))

    # Test 8: Check if Fences are Described Correctly
    if str(fence3) == "Fence(area=87.515, temperature=27, habitat=Jungle)\nwith animals:\nAnimal(name=Dumbo, species=Elephant, age=10, height=3.06, width=4.08, preferred_habitat=Jungle)":
        score += 1
    else:
